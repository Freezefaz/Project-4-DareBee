...
class TestCartView(TestCase):
    ...
    def test_add_to_cart_page(self):
        ...
        self.assertEqual(response.status_code, 200)
        session = self.client.session.get("shopping_cart")
        for k, e in session.items():
            exercise_added = e
        self.assertEqual(exercise_added["id"], str(exercise.id))
        self.assertEqual(exercise_added["title"], str(exercise.title))
        self.assertEqual(exercise_added["price"],
                         '${:.2f}'.format(int(exercise.price/100)))
        self.assertEqual(exercise_added["exercise_type"],
                         str(exercise.exercise_type))
